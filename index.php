...
?>
    <div id="home" class="current">
      ...
      <ul class="rounded">
        <li><a href="#maps"><img src="images/world.png" class="icon_list">Maps</a></li><!-- Added Maps list item IWW-->
      ...
      <ul class="rounded">
        ...
        <li><a href="#about"><img src="images/about.png" class="icon_list">About version <?=$arrSettings["mZabbixVersion"]?></a></li>
      </ul>

    <!-- Added Map to home screen: if mZabbixMapId setting is set IWW-->
    <?php
		if (isset($arrSettings["mZabbixMapId"])) {
			?>
			<ul class="rounded">
        <li><img src="map_img.php?sysmapid=<?=$arrSettings["mZabbixMapId"]?>" width="100%" /></li>
      </ul>
      <?php  }  ?>
...
<!-- // Begin Map Additions ###################################################-->
      ...
      <ul class="rounded">
      <?php
      $zabbixMaps = $zabbix->getMaps();
      if (isset($zabbixMaps)) {
          foreach ($zabbixMaps as $map) {
            // Start list item
            $mapCount = isset($map) ? count($map) : 0;
            $linkMap = "map.php?sysmapid=" . $map->sysmapid;
            $mapid = $map->sysmapid;
            if ($mapCount > 0) {
                            ?>
              <li><a href="<?=$linkMap?>">
               <img src="images/world.png" class="icon_list">
                 <?=$map->name?>
                                </a> 
              </li>
                            <?php
